 ...
 *
 * Returns business performance metrics for all businesses in the county.
 */

import { NextRequest, NextResponse } from 'next/server'
import { requireAdminCountyAccess, hasCountyAccess, CountyErrors } from '@/lib/county'
import { getBusinessesAnalytics } from '@/lib/analytics/admin-analytics'
...
export async function GET(request: NextRequest) {
  ...
  // Get analytics
  const result = await getBusinessesAnalytics(county.id, timeframe, limit, offset)
